...
function partition (arr, left, right) {
  ...
  for (let i = left + 1; i <= right; i++) {
    if (arr[i] < arr[left]) {
      let tmp = arr[i]; arr[i] = arr[placeIdx]; arr[placeIdx] = tmp;
      placeIdx++;
    }
  }
  let tmp = arr[left]; arr[left] = arr[placeIdx - 1]; arr[placeIdx - 1] = tmp;
  return placeIdx - 1;
}
...
var quickSortExample = function (arr, begin, end) {
  let partitionIndex,
    left = typeof begin === "number" ? begin : 0,
    right = typeof end === "number" ? end : arr.length - 1;
  if (left < right) {
...
function partitionExample (arr, left, right) {
  ...
  for (let i = index; i <= right; i++) {
    if (arr[i] < arr[pivot]) {
      let temp = arr[i]; arr[i] = arr[index]; arr[index] = temp;
      index++
    }
  }
  let temp = arr[pivot]; arr[pivot] = arr[index - 1]; arr[index - 1] = temp;
  return index - 1
}
